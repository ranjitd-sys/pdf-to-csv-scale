...
export default function ZipMorph() {
  ...
  const handleExecute = async () => {
    ...
    formData.append("tragetType", targetType);
    ...
    try{
      ...
      if(response.ok){
        ...
        alert("Initied"+ data.message)
      }
    }
    catch(error){
      console.log("Upload Filed", error);
      alert("DeepEcom Engine Filed");
    }finally{
        ...
        <div className="border-2 border-black">
          ...
          <div className="grid md:grid-cols-2">
            ...
            <div className="p-8">
              ...
              <div className="space-y-3">
                ...
                >
                  <span
                    className={`text-xs uppercase ${paramMode === "fixed" ? "font-bold underline" : ""}`}
                  >
                    Fixed (Standard)
                  </span>
                  <div
                    className={`w-3 h-3 border border-black rounded-full ${paramMode === "fixed" ? "bg-black" : ""}`}
                  />
                </button>
                <button
                ...
                >
                  <span
                    className={`text-xs uppercase ${paramMode === "custom" ? "font-bold underline" : ""}`}
                  >
                    Custom (Schema)
                  </span>
                  <div
                    className={`w-3 h-3 border border-black rounded-full ${paramMode === "custom" ? "bg-black" : ""}`}
                  />
                </button>
              </div>
            </div>
          ...
          >
            <span className="text-sm font-bold uppercase tracking-[0.2em]">
              {isUploading ? "Execute Internal Conversion" : "Awaiting Archive"}
            </span>
            
            <ArrowRight
              size={20}
              className={
                file ? "group-hover:translate-x-2 transition-transform" : ""
              }
            />
          </button>
        </div>
